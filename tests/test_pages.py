...
def test_quotes_list_shows_open_and_converted(client, ids, health_policy):
    ...
    assert "2 shown" in page and "1 still open" in page
    ...
    assert "No quotes" in client.get("/quotes?product=MOTOR").text
...
def test_customer_360_with_no_policies_and_unknown_id(client, ids):
    page = client.get(f"/customers/{ids['customers']['Rohan Das']}")
