...
def issue_policy(payload: PolicyCreate, session: Session) -> Policy:
    ...
    if quote.policy is not None:
        raise HTTPException(status.HTTP_409_CONFLICT, f"Quote already issued as {quote.policy.policy_number}")
    registration = (payload.vehicle_registration or "").strip().upper() or None
    if quote.product.code == ProductCode.MOTOR and not registration:
