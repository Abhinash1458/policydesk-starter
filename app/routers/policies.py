...
def issue_policy(payload: PolicyCreate, session: Session) -> Policy:
    ...
    if quote.policy is not None:
        raise HTTPException(status.HTTP_409_CONFLICT, "Quote already has a policy")

    if quote.product.code == ProductCode.MOTOR and not payload.vehicle_registration:
        raise HTTPException(
            status.HTTP_422_UNPROCESSABLE_ENTITY,
            "Vehicle registration is required for Motor policies",
        )
    ...
    policy = Policy(
        ...
        vehicle_registration=payload.vehicle_registration or None,
...
def update_policy_status(
    ...
    if policy.status == PolicyStatus.CANCELLED:
        raise HTTPException(
            ...
            "Cancelled policy cannot be changed",
        )
