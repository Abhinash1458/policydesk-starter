...
def update_claim_status(
    ...
    if claim.status in (ClaimStatus.APPROVED, ClaimStatus.REJECTED):
        raise HTTPException(
            ...
            "Final claim status cannot be changed",
        )

    # A claim cannot skip the review stage
    if claim.status == ClaimStatus.FILED and payload.status == ClaimStatus.APPROVED:
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "Claim must be Under Review before approval",
        )

    # Approval cannot exceed the remaining cover
