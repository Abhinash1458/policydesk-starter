...
def calculate_premium(
    *,
    ...
    if min_sum_insured is not None and sum_insured < min_sum_insured:
        raise PricingError(f"Sum insured must be at least {min_sum_insured:,.0f} for {product.value}")
    if max_sum_insured is not None and sum_insured > max_sum_insured:
        raise PricingError(f"Sum insured cannot exceed {max_sum_insured:,.0f} for {product.value}")
