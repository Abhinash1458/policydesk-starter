...
def age_factor(age: int, product: ProductCode) -> float:
    ...
    # TODO (Lab 1): four age bands; Motor and non-Motor differ only under 25; negative age -> PricingError
...
def tenure_factor(tenure_years: int) -> float:
    ...
    # TODO (Lab 1): look the tenure up in TENURE_FACTORS; anything else -> PricingError
...
def add_on_factor(product: ProductCode, add_ons: list[str]) -> float:
    ...
    # TODO (Lab 1): 1.0 + each valid loading from ADD_ONS[product]; blanks ignored; unknown add-on -> PricingError
...
def calculate_premium(
    *,
    ...
    # TODO (Lab 1): validate sum_insured, multiply the factors, apply MIN_PREMIUM, round to 2 dp
